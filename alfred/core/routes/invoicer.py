...
def generate_invoice(user, startDate, endDate):
    total = 0
    q = {
        'email': user,
        '_id': {
    ...
    for page in range(ceil(len(renders) / lines) or 1):
        ...
        pdf = setDate(pdf)
        pdf = setBillTo(pdf, user)
        pdf = setInvoiceNumber(
            pdf, invoiceNumber, page=page if len(renders) > lines else None
        )
        ...
        for i, render in enumerate(renders[page * lines : (page + 1) * lines]):
            pdf.drawString(
                x=left, y=top - lineHeight * (i + 1), text=render['filename']
            )
            if total < weeklyMax:
                total += unitPrice
                pdf.drawString(
                    x=right, y=top - lineHeight * (i + 1), text=f'$ {unitPrice:.2f}'
                )
        # draw subtotal
...
def setInvoiceNumber(pdf, invoiceNumber, page=None):
    pdf.drawString(
        x=right,
        y=top + 78,
        text=invoiceNumber + ('-' + str(page) if page is not None else ''),
    )
    return pdf
